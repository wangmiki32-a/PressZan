...
4. 每次确认点赞后，在同一作品评论 `👍👍👍`。评论区可能同时存在主评论框和回复框；只选择当前可见的顶层主评论框。先按当前账号稳定身份与完全相同文本确认本人评论；他人的相同文本不算。没有时只提交一次，本人评论可见后记录 `outgoing_comment_confirmed`。评论区不可用或状态不明确时立即安全暂停。
