...
维护 `500px-feedback-growth` 项目级 skill：以页面确认的互动和可重建事件日志为证据，安全执行 500px 点赞批次，并持续学习哪些摄影师更可能在 72 小时内产生归因回馈。
...
3. 涉及算法、状态或 Dashboard 时读取 [docs/architecture.md](docs/architecture.md)。
...
- 首次真实互动必须来自用户明确批准且仍有效的 preview；录制工作流不等于批准候选。
...
- 每次显式 `run` 最多完成 25 个页面确认的成功点赞；Asia/Shanghai 每日最多 100 个，未完成额度不结转。
